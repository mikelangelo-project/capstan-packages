...
cd ${OSV_BUILD_DIR}
echo "/usr/bin/pisoFoam.so: ${OSV_DIR}/mike-apps/OpenFOAM/ROOTFS/usr/bin/pisoFoam.so" > usr.manifest
...
cp ${OSV_DIR}/mike-apps/OpenFOAM/ROOTFS/usr/bin/pisoFoam.so ${PACKAGE_RESULT_DIR}/usr/bin

cd ${PACKAGE_RESULT_DIR}
capstan package init --name "${PACKAGE_NAME}" \
    --title "OpenFOAM pisoFoam" \
